...
def test_hill_climbing_greedy(area: Rail_NL, amount_trajects: int, max_time: int, amount_stations: int, time_to_run: float) -> None:
    """
    Test the hill climbing greedy algorithm with different parameters and generate a plot.
    ...
    for i in range(len(list_amount_trajects)):
        for j in range(len(list_amount_neighbors)):
            ...
            while True:
                ...
                file_name = f'experiments/hill_climbing_greedy/pickle/results_{list_amount_trajects[i]}_{list_amount_neighbors[j]}.pickle'
    ...
    for q in range(len(list_amount_trajects)):
        ...
        for r in range(len(list_amount_neighbors)):
            ...
            file = open(f'experiments/hill_climbing_greedy/pickle/results_{list_amount_trajects[q]}_{list_amount_neighbors[r]}.pickle', 'rb')
    ...
    plt.title("Graph of mean score, using the hill climbing greedy algorithm, against amount of neighbors, for different amount of stations (run for 1 minute per starting combination)")
    plt.legend()
