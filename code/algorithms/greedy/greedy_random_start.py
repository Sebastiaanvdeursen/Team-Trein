...
def run_greedy_random(Area: Rail_NL, amount_trajects: int, max_time: int,
    ...
    """
    runs the greedy algorithm with random starting stations,
    runs removing_lines and remove_end to improve the trajects

    pre:
        - area object of type Rail_NL both small or large
        - amount_trajects is an int corresponding to the amount
          of trajects allowed
        - max_time is an int corresponding to the amount of time
          allowed per traject
        - used_for_hill_climbing is a bool
        - printed is a bool, if positive the trajects are printed
          out instead of returned
        - info is a bool that returns that makes the function
          return the trajects too
...
def run_greedy_track_random(Area: Rail_NL, amount_stations: int, max_time: int, list_stations: list[str],
                            used_for_hill_climbing: bool = False,
                            printed: bool = True, start: int = -1) -> tuple[int, Rail_NL, Traject]:
    """
    ...
    while True:
        ...
        for i in range(len(random_traject.current_station.connections)):
            if random_traject.current_station.connections[list_stations_current[i]].done:
                if going_back == "" or (random_traject.current_station.connections[going_back].time >
        ...
        if destination == "":
            ...
            if going_back == "" or (random_traject.total_time +
                                    random_traject.current_station.connections[going_back].time
                                    > max_time):
                break
            else:
        ...
        # check if selected destination is possible, if possible move there otherwise break
            ...
            added_time = random_traject.current_station.connections[destination].time
            if random_traject.total_time + added_time > max_time:
                break
